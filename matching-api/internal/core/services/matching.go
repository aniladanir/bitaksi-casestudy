...
type MatchingService interface {
	FindNearestDriverLocation(ctx context.Context, userLocation domain.UserLocation, authToken string) (*domain.DriverLocation, *domain.Distance, error)
}
...
func (ds *matchingService) FindNearestDriverLocation(ctx context.Context, userLocation domain.UserLocation, authToken string) (*domain.DriverLocation, *domain.Distance, error) {
	driverLocation, distanceToUser, err := ds.locationFinder.GetNearestDriverLocation(ctx, userLocation, authToken)
	if err != nil {
		return nil, nil, err
	}
