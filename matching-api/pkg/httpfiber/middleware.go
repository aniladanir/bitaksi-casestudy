...
func TracingMiddleware(ctx fiber.Ctx) error {
	...
	ctx.Context().SetUserValue(CtxKeyTraceID, ctx.Get(HeaderXTraceID))
	ctx.Context().SetUserValue(CtxKeyRequestID, ctx.Get(HeaderXRequestID))
