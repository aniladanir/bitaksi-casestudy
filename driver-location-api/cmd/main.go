...
func InitializeComponents(ctx context.Context) (*httphandler.Handler, error) {
	...
	mongoDB := mongodb.GetDatabase(mongoClient, config.GetDBName())

	// create repositories
	locationRepo := repositories.NewLocationRepository(mongoDB)

	// create authenticator
	matchingApiUrl, err := url.Parse(config.GetRemoteUrl("matchingApi"))
	if err != nil {
		return nil, fmt.Errorf("could not parse driver location api url: %w", err)
	}
	matchingApiClient := authenticator.NewMatchingApiClient(
		*matchingApiUrl,
		config.GetRemoteVersion("matchingApi"),
