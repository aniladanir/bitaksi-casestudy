...
func (lr *locationRepository) GetNearestDriverLocation(ctx context.Context, location domain.DriverLocation, radius float64) (*domain.DriverLocation, error) {
	filter := bson.D{{Key: "location", Value: bson.D{
		{Key: "$near", Value: bson.D{
			{Key: "$geometry", Value: bson.D{
				{Key: "type", Value: location.Location.Type},
				{Key: "coordinates", Value: location.Location.Coordinates},
			}},
			{Key: "$maxDistance", Value: radius}, // Maximum distance in meters
		}},
	}},
	}
