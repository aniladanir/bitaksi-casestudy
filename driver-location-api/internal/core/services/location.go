...
import (
	...
	"github.com/aniladanir/bitaksi-casestudy/matching-api/pkg/haversine"
)

type LocationService interface {
	CreateOrUpdateDriverLocations(ctx context.Context, locations []domain.DriverLocation) error
	FindNearestDriverDistance(ctx context.Context, location domain.DriverLocation, searchRadius float64, authToken string) (*domain.DriverLocation, *domain.Distance, error)
...
func (ls *locationService) FindNearestDriverDistance(ctx context.Context, userLocation domain.DriverLocation, searchRadius float64, authToken string) (*domain.DriverLocation, *domain.Distance, error) {
	driverLocation, err := ls.locationRepo.GetNearestDriverLocation(ctx, userLocation, searchRadius)
	if err != nil {
		return nil, nil, err
	}

	distanceKM, err := haversine.HaversineDistanceInKM(driverLocation.Location, userLocation.Location)
	if err != nil {
